...
const SUPPORT_ITEMS = [
  {
    ...
    description:
      "Liên hệ Tễu Tạc để được kiểm tra và hỗ trợ bổ sung hoặc thay thế linh kiện theo chính sách.",
  },
  {
    icon: "support_agent",
    title: "Cần hỗ trợ lắp ráp?",
    description:
      "Xem hướng dẫn qua QR/video hoặc liên hệ các kênh hỗ trợ khách hàng của Tễu Tạc.",
  },
];
  ...
  if (!product) {
    return (
      <div className="mx-auto max-w-container-max px-gutter py-section-gap text-center md:px-margin-desktop">
        ...
        <Link
          to="/products"
          className="mt-4 inline-block font-label-lg text-label-lg text-secondary underline"
        >
          Quay lại cửa hàng
        </Link>
      </div>
    ...
    <div className="mx-auto max-w-container-max px-gutter py-section-gap md:px-margin-desktop">
        ...
        <div className="flex flex-col">
          {product.qrCodeUrl && (
            <div className="mb-6 flex flex-col items-center gap-3">
              <div className="ghost-border rounded-lg bg-surface-container-low p-2">
                <img
                  src={product.qrCodeUrl}
                  alt={`Mã QR xem mô hình 3D ${product.name}`}
              ...
              <div className="text-center">
                ...
                <p className="font-body-md text-body-md text-on-surface-variant">
                  Xem mô hình 3D của {product.name}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="mb-8 flex items-center gap-6">
            <div className="ghost-border flex items-center rounded bg-surface-container-low">
              ...
              <span className="px-4 font-body-md text-body-md text-on-surface">
                {quantity}
              </span>
              <button
                type="button"
      ...
      <div className="mb-section-gap grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="ghost-border bg-surface-container-low p-8 md:col-span-2">
          <h2 className="mb-4 font-headline-sm text-headline-sm text-secondary">
            Câu chuyện nhân vật
          </h2>
        ...
        <div className="ghost-border flex flex-col justify-between bg-surface-container-low p-8">
          <div>
            <h2 className="mb-4 font-headline-sm text-headline-sm text-secondary">
              Cách Tễu chuyển động
            </h2>
            <p className="mb-6 font-body-md text-body-md text-on-surface-variant">
              Điều khiển Tễu bằng cơ chế dây treo và chuyển động tay đơn giản
          ...
          <div className="space-y-4">
            {SPECS.map((spec) => (
              ...
              >
                ...
                <span className="font-body-md text-body-md text-inverse-surface">
                  {spec.value}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-4">
            {SUPPORT_ITEMS.map((item) => (
              <div key={item.title} className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-label-lg text-label-lg text-inverse-surface">
                    {item.title}
                  </h3>
      ...
      <div className="ghost-border mb-section-gap bg-surface-container-low p-8">
        <h2 className="mb-6 text-center font-headline-md text-headline-md text-secondary">
          Hướng dẫn lắp ráp
        </h2>
